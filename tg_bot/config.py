...
class Settings:
    ...
    donationalerts_base_url: str  # e.g. https://www.donationalerts.com/r/antigaz
...
def load_settings() -> Settings:
    ...
    base_url = os.getenv("TRACKRATER_BASE_URL", "http://127.0.0.1:5000").strip().rstrip("/")
    api_token = os.getenv("TRACKRATER_TG_API_TOKEN", "").strip()
    if not api_token:
        raise RuntimeError("TRACKRATER_TG_API_TOKEN is required")

    required_chats = []
    for item in _split_csv(os.getenv("TG_REQUIRED_CHATS", "")):
        try:
            required_chats.append(int(item))
        except Exception:
            # allow @username, bot will still use it for join links; check uses chat_id only
            pass
